...
function SigninCeo() {
	...
	const onSubmit = async (values) => {
		...
		const creds = {
			ceo_email: values?.email,
			pass: values?.password,
		}

		await axios
			.post(`${url}/ceo/ceo_login`, creds)
			.then((res) => {
				if (res?.data?.suc === 1) {
	...
	return (
		<div className="bg-slate-700 p-20 flex justify-center min-h-screen min-w-screen">
			<motion.div
				initial={{ opacity: 0 }}
				animate={{ opacity: 1 }}
                ...
                `}
					>
						<div className="flex-col items-center justify-center ml-7 2xl:ml-36 2xl:mt-44">
							<motion.h2
								className="text-slate-800 text-4xl mt-14 ml-24 font-bold"
								initial={{ opacity: 1 }}
						...
						>
							...
							<div className="pt-2">
								<Link to={routePaths.FORGOTPASS}>
									<p className="text-xs text-slate-800 hover:underline py-2 cursor-pointer">
										Forgot password?
									</p>
								</Link>
							</div>
							<Spin
								...
								size={5}
								className="text-slate-800 w-52 dark:text-gray-400"
								spinning={loading}
							>
								<div className="pt-4 pb-4 flex justify-center text-sm">
									<button
										...
										type="submit"
										className="bg-slate-800 hover:duration-500 w-full hover:scale-105  text-white p-3 rounded-full"
									>
			...
			>
				...
				>
					<div className="flex-col items-center justify-center">
						<motion.h2
							className="text-slate-800 text-4xl mt-14 mx-24 font-bold"
							initial={{ opacity: 1 }}
							animate={{ opacity: 0, y: -20 }}
							transition={{ delay: 4, type: "tween" }}
						>
					...
					>
						...
						<div className="pt-2">
							<Link to={routePaths.FORGOTPASS}>
								<p className="text-xs text-slate-900 hover:underline py-2 cursor-pointer">
									Forgot password?
								</p>
							</Link>
						</div>
