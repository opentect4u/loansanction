...
function EditLoanFormCr() {
	...
	const [commentsCreditManager, setCommentsCreditManager] = useState("")
	...
	const onSubmit = async (values) => {
		...
		data.append(
			"user_id",
			+JSON.parse(localStorage.getItem("cr_mgr_details"))?.id
		)
		...
		data.append(
			"created_by",
			JSON.parse(localStorage.getItem("cr_mgr_details"))?.created_by
		)
	...
	const handleReject = async (appStatus, e) => {
		...
		const creds = {
			...
			application_no: +params.id,
			user_id: JSON.parse(localStorage.getItem("cr_mgr_details")).id,
			fwd_brn: JSON.parse(localStorage.getItem("cr_mgr_details")).branch_code,
			loan_appr_id: forwardedById,
			brn_remarks: commentsCreditManager,
			application_status: appStatus,
			brn_mgr_id: JSON.parse(localStorage.getItem("cr_mgr_details")).id,
			created_by:
				JSON.parse(localStorage.getItem("cr_mgr_details")).first_name +
				" " +
				JSON.parse(localStorage.getItem("cr_mgr_details")).last_name,
		}
		...
		console.log(
			"ooooooooooooooo------------------",
			+JSON.parse(localStorage.getItem("cr_mgr_details"))?.user_type
		)

		await axios
			.post(`${url}/credit/credit_manager_reject`, creds)
			.then((res) => {
	...
	const sendToCEO = async (appStatus) => {
		...
		setLoading(true)
		const creds = {
			brn_code: JSON.parse(localStorage.getItem("cr_mgr_details")).branch_code,
			credit_brn_code: JSON.parse(localStorage.getItem("cr_mgr_details"))
				.branch_code,
			application_no: +params.id,
			credit_user_id: JSON.parse(localStorage.getItem("cr_mgr_details")).id,
			// mng_user_id: JSON.parse(localStorage.getItem("cr_mgr_details")).id,
			// mng_brn_code: JSON.parse(localStorage.getItem("cr_mgr_details"))
			// 	.branch_code,
			// credit_mgr_id: creditManagerId,
			remarks: commentsCreditManager,
			application_status: appStatus,
		}
	...
	return (
		<>
			<Sidebar mode={2} />
			<section className="bg-teal-50 dark:bg-[#001529] flex justify-center align-middle p-5">
			...
			/> */}
				...
				<div className=" bg-white p-5 w-4/5 min-h-screen rounded-3xl">
					...
					<div className="w-auto mx-14 my-4">
						<FormHeader text="Pending Application Preview & Edit" mode={2} />
					</div>
					<Spin
								...
								<form onSubmit={handleSubmit}>
									<div className="card flex flex-col justify-center px-16 py-5">
										...
										<Spin
											...
											spinning={loading}
										>
											<div className="mt-10 text-teal-800">
										...
										{rejectReasonsArray?.length > 0 && (
											<div className="mt-14 bg-teal-50 rounded-xl p-10">
												<div className="text-lg font-bold text-teal-800 mb-4">
											...
											<div className="mt-7">
												...
												<div className="mt-7">
													<TDInputTemplateCr
														...
														label={`Comments`}
														name="comments"
														formControlName={commentsCreditManager}
														handleChange={(e) =>
															setCommentsCreditManager(e.target.value)
														}
														mode={3}
													/>
													{!commentsCreditManager ? (
														<VError
															title={
										...
										{loanApproveStatus !== "A" && loanApproveStatus !== "R" ? (
											...
											<Tag
												...
												className="mt-10 p-5 rounded-lg text-xl font-bold self-center"
											>
												E-Files rejected and sent to Branch Manager.
											</Tag>
										) : (
											<Tag
												color="red"
