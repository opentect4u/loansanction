...
function TDInputTemplateCeo(props) {
	return (
		<>
			<label
				htmlFor={props.name}
				className="block mb-2 text-sm capitalize font-bold text-slate-700 dark:text-gray-100"
			>
				{/* <div class="relative">
    ...
    <label for="floating_outlined" class="absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-white dark:bg-gray-900 px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto start-1">Floating outlined</label>
			...
			{props.mode == 1 && (
				<input
					...
					accept={props.accept}
					max={props.max}
					className="bg-white border-gray-400 text-gray-800 text-sm rounded-md  focus:border-teal-800 active:border-slate-600 focus:ring-slate-600 focus:border-1 duration-500 block w-full p-2 dark:bg-bg-white dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
			...
			{props.mode == 2 && (
				...
				<select
					id="countries"
					className="bg-white border-1 border-gray-400 text-gray-800 text-sm rounded-lg focus:border-slate-800 active:border-slate-600 focus:ring-slate-600 focus:border-1 duration-500 block w-full p-2 dark:bg-bg-white dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
					value={props.formControlName}
					onChange={props.handleChange}
					name={props.name}
					placeholder={props.placeholder}
			...
			{props.mode == 3 && (
				<textarea
					rows="4"
					className="bg-white border-1 border-gray-400 text-sm rounded-lg  focus:border-slate-800 active:border-slate-600 focus:ring-slate-600 focus:border-1 duration-500 block w-full p-2 dark:bg-bg-white dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
					name={props.name}
					value={props.formControlName}
					placeholder={props.placeholder}
					onChange={props.handleChange}
...
export default TDInputTemplateCeo
